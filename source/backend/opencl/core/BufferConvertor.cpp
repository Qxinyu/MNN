...
bool convertGLMemBetweenCLmem(const Tensor *input, const Tensor *output, OpenCLRuntime *runtime, bool toDevice, bool toHost) {
    ...
    if(toDevice){
        kernelW = runtime->buildKernelWithCache("glmem_convert", "gl_to_cl", buildOptions, input, output);
        outputShape = tensorShapeFormat(output);
    } else if(toHost){
        kernelW = runtime->buildKernelWithCache("glmem_convert", "cl_to_gl", buildOptions, input, output);
    ...
    if(toDevice || runtime->getGpuMemType() == IMAGE) {
    ...
    if(toHost || runtime->getGpuMemType() == IMAGE) {
